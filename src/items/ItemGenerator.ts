...
import { ITEMS, AFFIXES, RARITY_AFFIX_COUNT, getRarityWeights } from '../data/items';
...
export function resetPity(): void { pityEpic = 0; pityLegendary = 0; }

export function rollRarity(floor: number): Rarity {
  const weights = { ...getRarityWeights(floor) };
  if (pityEpic >= 15) weights.epic += 8;
  if (pityEpic >= 20) weights.epic += 12;
  if (pityLegendary >= 25) weights.legendary += 5;
  if (pityLegendary >= 40) weights.legendary += 10;
  ...
  for (const [rar, w] of Object.entries(weights) as [Rarity, number][]) {
    ...
    if (r <= 0) {
      ...
      } else { pityEpic++; pityLegendary++; }
      return rar;
    }
  }
  pityEpic++; pityLegendary++;
  return 'common';
}
...
function pickAffixes(count: number, slot: EquipmentSlotType): AffixInstance[] {
  ...
  for (let i = 0; i < count && pool.length > 0; i++) {
    ...
    result.push({ id: def.id + '_' + Date.now() + i, name: def.name, stat: def.stat, value });
  }
  return result;
}

function scaleItemPower(def: ItemDefinition, floor: number) {
  ...
  if (def.baseStats) {
    for (const [k, v] of Object.entries(def.baseStats)) stats[k] = Math.floor(v * scale);
  }
  return { atkMin, atkMax, stats };
}

export function generateItem(floor: number, forcedRarity?: Rarity, forcedSlot?: EquipmentSlotType): ItemInstance {
  ...
  if (candidates.length === 0) def = ITEMS.rustbound_sword;
  else {
...
export function generateBossLoot(floor: number): ItemInstance[] {
  ...
  if (Math.random() < 0.15 + floor * 0.02) items.push(generateItem(floor, 'legendary'));
  return items;
}

export function getSellPrice(item: ItemInstance): number {
  const base: Record<Rarity, number> = { common: 8, uncommon: 25, rare: 60, epic: 150, legendary: 400, unique: 500 };
